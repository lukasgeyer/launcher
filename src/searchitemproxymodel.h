...
#ifndef SEARCHITEMPROXYMODEL_H
#define SEARCHITEMPROXYMODEL_H
...
/*!
 * \brief A proxy model providing a flat search item model from an item model.
 */
class SearchItemProxyModel : public QAbstractProxyModel
{
public:
   /*!
    * Constructs a proxy model with the parent \a parent.
    */
   SearchItemProxyModel(QObject* parent = nullptr);
...
private:
    ...
    */
   bool isValid_(int row, int column) const
   {
    ...
    */
   bool isValid_(const QModelIndex& proxyIndex) const
   {
      return ((proxyIndex.isValid()) && isValid_(proxyIndex.row(), proxyIndex.column()));
   }
};

#endif // SEARCHITEMPROXYMODEL_H
